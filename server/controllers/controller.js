...
module.exports = {
	getData: async (req, res) => {
		const db = req.app.get('db'),
			states = await db.get_state_ranks().catch(err => console.log(err)),
		...
		output.mapViews = req.session.mapViews
								...
								data: stateData
									...
									.map(d => {
										return {
											race: d.race,
											value: d.iat_avg,
											label: `${d.race}\n IAT Race Score of ${Number.parseFloat(
												d.iat_avg
											).toPrecision(3)} \nBased on ${
												d.iat_responses
								...
								data: stateData
									...
									.map(d => {
										return {
											...
											label: `Per 100,000 based on a\n 2018 year-end prison population of ${d.prison_pop_count}\n and a state ${d.race}\n population of ${d.population}.`
										}
									})
							}
						]
